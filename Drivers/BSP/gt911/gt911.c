...
int8_t gt911_i2c_bus_id = -1;
static uint16_t touch_gt911_hor = 0;
static uint16_t touch_gt911_ver = 0;
...
{
    var ? HAL_GPIO_WritePin(TOUCH_IIC_SDA_GPIO_Port, TOUCH_IIC_SDA_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(TOUCH_IIC_SDA_GPIO_Port, TOUCH_IIC_SDA_Pin, GPIO_PIN_RESET);
}
...
{
    var ? HAL_GPIO_WritePin(TOUCH_IIC_CLK_GPIO_Port, TOUCH_IIC_CLK_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(TOUCH_IIC_CLK_GPIO_Port, TOUCH_IIC_CLK_Pin, GPIO_PIN_RESET);
}
...
{
    sta ? HAL_GPIO_WritePin(TOUCH_INT_GPIO_Port, TOUCH_INT_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(TOUCH_INT_GPIO_Port, TOUCH_INT_Pin, GPIO_PIN_RESET);
}
void touch_rst_write(uint8_t sta)
{
    sta ? HAL_GPIO_WritePin(TOUCH_RST_GPIO_Port, TOUCH_RST_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(TOUCH_RST_GPIO_Port, TOUCH_RST_Pin, GPIO_PIN_RESET);
}
...
void gt9xx_init(void)
{
    ...
    touch_gt911_hor = *(uint16_t *)&GT911_Info[6];
    touch_gt911_ver = *(uint16_t *)&GT911_Info[8];
...
#if TOUCH_GT911_ROT == 3
    for (uint8_t i = 0; i < 5; i++)
    {
        temp = TouchPoints[i].px;
        TouchPoints[i].px = touch_gt911_ver - TouchPoints[i].py - 1;
        TouchPoints[i].py = temp;
    }
#else
